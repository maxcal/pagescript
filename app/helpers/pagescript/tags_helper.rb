module Pagescript
  module TagsHelper
    # Generates a `<body>` tag with HTML5 data attributes for the controller name
    # and action. The keyword arguments are forwarded to tag and content_tag.
    ...
    def body_tag(**kwargs, &block)
      ...
      if block_given?
        content_tag(:body, block.call, options)
      else
        tag(:body, options, true).html_safe
      end
    end
  end
end
